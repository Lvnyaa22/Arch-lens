...
import { buildFlowElements, getLayoutedElements } from '../src/components/diagram/layout-dagre';
import { buildChatSystemPrompt, buildAdHocFlowPrompt } from '../src/lib/ai/prompts';
...
async function runEndToEndVerification() {
  ...
  assert(DEMO_PROJECT_SHOPSTREAM.id === 'demo-shopstream-cloud', 'ShopStream Demo Project ID is valid');
  assert(DEMO_PROJECT_SHOPSTREAM.components.length === 10, 'ShopStream contains exactly 10 microservice components');
  assert(DEMO_PROJECT_SHOPSTREAM.connections.length === 9, 'ShopStream contains 9 inter-service connections');
  assert(DEMO_PROJECT_SHOPSTREAM.databases.length === 3, 'ShopStream contains 3 persistence stores (PostgreSQL, MongoDB, Redis)');
  assert(DEMO_PROJECT_SHOPSTREAM.apis.length === 3, 'ShopStream contains 3 documented API endpoints');
  assert(DEMO_PROJECT_SHOPSTREAM.data_flows.length >= 1, 'ShopStream contains pre-analyzed order placement flow');
  assert(DEMO_PROJECT_SHOPSTREAM.chunks!.length === 4, 'ShopStream contains 4 indexed document chunks');

  assert(DEMO_PROJECT_COLLABCANVAS.id === 'demo-collabcanvas-realtime', 'CollabCanvas Demo Project ID is valid');
  assert(DEMO_PROJECT_COLLABCANVAS.components.length === 8, 'CollabCanvas contains 8 realtime components');
  assert(DEMO_PROJECT_COLLABCANVAS.connections.length === 7, 'CollabCanvas contains 7 communication edges');
  assert(DEMO_PROJECT_COLLABCANVAS.databases.length === 3, 'CollabCanvas contains 3 storage entities (Redis, Postgres, S3)');
  assert(DEMO_PROJECT_COLLABCANVAS.chunks!.length === 3, 'CollabCanvas contains 3 indexed document chunks');

  // 2. ARCHITECTURE GRAPH & DAGRE AUTO-LAYOUT
  console.log('\n[SECTION 2]: Architecture Topology Graph & Dagre Layout');
  const shopstreamLayoutLR = buildFlowElements(DEMO_PROJECT_SHOPSTREAM.components, DEMO_PROJECT_SHOPSTREAM.connections, 'LR');
  ...
  const collabLayoutTB = buildFlowElements(DEMO_PROJECT_COLLABCANVAS.components, DEMO_PROJECT_COLLABCANVAS.connections, 'TB');
  ...
  const chunksSS = DEMO_PROJECT_SHOPSTREAM.chunks!;

  // Q1: Authentication
  const q1 = 'How does authentication work?';
  const ret1 = retrieveRelevantChunks(q1, chunksSS, 3);
  ...
  const ret2 = retrieveRelevantChunks(q2, chunksSS, 3);
  ...
  const ret3 = retrieveRelevantChunks(q3, chunksSS, 3);
  ...
  const ret4 = retrieveRelevantChunks(q4, chunksSS, 3);
  ...
  const ret5 = retrieveRelevantChunks(q5, chunksSS, 3);
  ...
  const devPrompt = buildChatSystemPrompt('{}', 'context', 'developer');
  ...
  const begPrompt = buildChatSystemPrompt('{}', 'context', 'beginner');
  ...
  const intPrompt = buildChatSystemPrompt('{}', 'context', 'interview');
  ...
  const flow = DEMO_PROJECT_SHOPSTREAM.data_flows[0];
  ...
  assert(Boolean(flow.steps[3].communication_method?.includes('AMQP')), 'Step 4 publishes via AMQP (RabbitMQ)');
  assert(flow.steps[4].component_id === 'payment-service', 'Step 5 settles in payment-service');

  // Ad-hoc prompt check
  const adHocPrompt = buildAdHocFlowPrompt('{}', 'doc context', 'How does user login work?');
  ...
  for (const q of searchQueries) {
    ...
    const matchComp = DEMO_PROJECT_SHOPSTREAM.components.filter((c) =>
      c.name.toLowerCase().includes(qLower) || c.description.toLowerCase().includes(qLower) || c.technology?.toLowerCase().includes(qLower)
    );
    const matchChunk = DEMO_PROJECT_SHOPSTREAM.chunks!.filter((c) =>
  ...
  // PDF
  const pdfDoc = await parseUploadedDocument(fs.readFileSync('test-docs/project-a-shopstream.pdf'), 'shopstream.pdf');
  assert(pdfDoc.pageCount === 4, 'PDF parsing extracts 4 pages with page markers');

  // Markdown
  const mdDoc = await parseUploadedDocument(Buffer.from('# Architecture\n\n### Service A\nNode.js service.\n\n### Service B\nPostgreSQL db.'), 'arch.md');
  assert(mdDoc.text.includes('Service A') && mdDoc.pageCount >= 1, 'Markdown parsing extracts headings and text');

  // Plain Text
  const txtDoc = await parseUploadedDocument(Buffer.from('CollabCanvas Architecture\nPage 1\nWebSocket server with Go.'), 'spec.txt');
  assert(txtDoc.text.includes('WebSocket') && txtDoc.pageCount >= 1, 'Plain text parsing works');

  // Source Code (TypeScript)
  const codeDoc = await parseUploadedDocument(Buffer.from('export interface User { id: string; email: string; }\nexport const login = () => {};'), 'user.ts');
  ...
  try {
    await parseUploadedDocument(Buffer.from('bin-data'), 'exploit.sh');
  ...
  assert(DEMO_PROJECT_SHOPSTREAM.project.technologies.frontend.length > 0, 'Report frontend technologies populated');
  assert(DEMO_PROJECT_SHOPSTREAM.project.technologies.backend.length > 0, 'Report backend technologies populated');
  assert(DEMO_PROJECT_SHOPSTREAM.project.technologies.database.length > 0, 'Report database technologies populated');
  assert(DEMO_PROJECT_SHOPSTREAM.unknowns.length > 0, 'Report unknowns matrix explicitly documents missing specs');
  assert(DEMO_PROJECT_SHOPSTREAM.observations.length > 0, 'Report architectural observations documented');
